...
# Constants
PERCENT_CENTER_RECT = 0.3  # For calculating the center rectangle's size
PERCENT_TARGET_RADIUS = 0.6 * PERCENT_CENTER_RECT  # Minimum target radius to follow
HOVERING_ALTITUDE = 3.0  # Altitude in meters to which the drone will perform its tasks
NUM_FILT_POINTS = 100  # Number of filtering points for the Moving Average Filter
...
target_person_image = 'init_frame.png'
target_person_info = 'person_bbox_coordinates.json'
# Workspace directory
...
VISION_SYSTEM_PARAMETERS = {
    'tracker_name': 'siamfcpp_alexnet_votlt',
...
class VisionSystem:
    ...
    async def get_target_coordinates(self, frame):
        ...
        if pred_bbox_confidence > 0.70:
            ...
            return {'width': person_bbox_center[0], #person_bbox_center[1], person_bbox_center[0]}
                    'height': person_bbox_center[1]}, frame, pred_bbox, selected_person_pose
        else:
...
class Drone:
    ...
    async def fly():
        ...
        # Start Computer Vision Sub-System
        ...
        async for state in uav.core.connection_state():
            if state.is_connected:
                logger.info(f"UAV target UUID: {state}")
        ...
        async for gps_info in uav.telemetry.gps_info():
            ...
            break
        logger.info("Establishing GPS lock on UAV..")
        ...
        logger.info("Fetching amsl altitude at home location....")
        ...
        logger.info("-- Setting initial set point")

        await uav.offboard.set_position_ned(PositionNedYaw(0.0, 0.0, 0.0, 0.0))

        logger.info("-- Starting off board")

        try:
            await uav.offboard.start()
        except OffboardError as error:
            logger.error(f"Starting off board mode failed with error code: {error._result.result}")
            logger.error("-- Disarming")
        ...
        while True:
            ...
            # cv2.waitKey(1)
            # cv2.imwrite(f"Candidate__frame{cnt}.png", frame)
            # Get the target coordinates (if any target was detected)
            # tgt_cam_coord, frame, contour = await get_target_coordinates(frame)
            tgt_cam_coord, frame, person_bbox, person_pose = await vision_sub_system.get_target_coordinates(frame)
            ...
            tgt_cart_coord = {'x': (tgt_filt_cam_coord['width'] - params['y_ax_pos']),
                              'y': (params['x_ax_pos'] - tgt_filt_cam_coord['height'])}
            # tgt_cart_coord = {'x': (params['y_ax_pos'] - tgt_filt_cam_coord['width']),
            #                   'y': (params['x_ax_pos'] - tgt_filt_cam_coord['height'])}
            # tgt_cart_coord = {'x': (tgt_filt_cam_coord['width'] - params['resized_width_center']),
            #                   'y': (tgt_filt_cam_coord['height'] - params['resized_height_center'])}
            # Compute scaling conversion factor from camera coordinates in pixel units
            # to Cartesian coordinates in meters
            COORD_SYS_CONV_FACTOR = 0.03
            # if abs(tgt_filt_cam_coord['width']) > params['resized_width_center'] or \
            #         abs(tgt_filt_cam_coord['height']) > params['resized_height_center']:
            #     # Compute North, East coordinates applying "camera pixel" to Cartesian conversion factor
            #     E_coord = tgt_cart_coord['width'] * COORD_SYS_CONV_FACTOR
            #     N_coord = tgt_cart_coord['height'] * COORD_SYS_CONV_FACTOR
            # If the target is outside the center rectangle, compute North and East coordinates cent_rect_half_width
            ...
            frame = await draw_objects(tgt_cam_coord, tgt_filt_cam_coord, frame, person_bbox, person_pose)
            # fps.update()
            # fps.stop()
            ...
            cv2.putText(frame, str(north_label), (5, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.putText(frame, str(east_label), (5, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.putText(frame, str(distance_label), (5, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.putText(frame, str(N_coord), (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.putText(frame, str(E_coord), (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.putText(frame, "FPS: {:.2f}".format(1/times[-1]), (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),
                        2)
            cv2.putText(frame, "elasped time: {:.2f}".format(fps.elapsed()), (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),
                        2)
            cv2.putText(frame, "FPS2: {:.2f}".format(fps.fps()), (10, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),
                        2)
            ...
            cv2.putText(frame, str(distance.current_distance_m), (80, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),
                        2)
            ...
            # cv2.imwrite(f"Candidate__frame2{cnt}.png", frame)

            # Catch aborting key from computer keyboard
            key = cv2.waitKey(1)  # & 0xFF
            # If the 'esc' key is pressed, break the 'while' infinite loop
            if key == 27:  # ord("q")
...
async def get_image_params(frame):
    ...
    dimension = (params['resized_width'], params['resized_height'])
    # frame = cv2.resize(frame, dimension, interpolation=cv2.INTER_AREA)
    params['resized_height_center'] = int(params['resized_height'] // 2)
    params['resized_width_center'] = int(params['resized_width'] // 2)
    ...
    # params['cent_rect_half_width'] = params['x_ax_pos'] # Use half percent (0.5)
    # params['cent_rect_half_height'] = params['y_ax_pos']  # Use half percent (0.5)
...
async def draw_objects(cam_coord, filt_cam_coord, frame, person_pred_box, person_pose):  # contour
    ...
    (x, y, w, h) = [int(v) for v in person_pred_box]
    ...
    if person_pose is not None:
        for person_keypoints in person_pose:
            cv2.circle(frame, (int(person_keypoints[0]), int(person_keypoints[1])),
                       3, (0, 255, 0), -1)
    else:
